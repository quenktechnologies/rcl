...
exports.code = function (n, ctx) {
    if (n instanceof nodes.File) {
        ...
        var args = (n.args.length > 0) ? n.args.map(function (x) { return exports.code(x, ctx); }).join(',') : '';
        return "" + target + (args ? '(' + args + ')' : '');
    }
    else if (n instanceof nodes.ControllerAction) {
        var target = exports.code(n.target, ctx);
        var member = exports.code(n.member, ctx);
        var args = (n.args.length > 0) ?
            n.args.map(function (x) { return exports.code(x, ctx); }).concat(['_req, _res']).join(',') : '_req, _res';
        if (ctx.has(target))
